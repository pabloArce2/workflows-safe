...
export const checkNodeValidity = ({
    ...
    if (functionInstance) {
        ...
        if (functionInstance.inputErrors.length > 0) {
            ...
            return invalid(`Input ${input.label} was connected with an incompatible value. ${trace.join(" ")}`)
        }
        ...
        for (const { outputId } of functionInstance.outputErrors) {
            ...
            return invalid(`Some inputs are incompatible with each other. ${output.neverReason ?? ""}`)
        }
    }
