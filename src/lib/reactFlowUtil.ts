...
export interface NodeProto {
    ...
    data: any //Omit<NodeData, "id" | "inputData"> & { inputData?: InputData }
    nodeType: NodeType
}
...
export const snapToGrid = (position: Readonly<XYPosition>, snapToGridAmount: number): XYPosition => ({
    x: position.x - (position.x % snapToGridAmount),
    y: position.y - (position.y % snapToGridAmount),
})

export const isSnappedToGrid = (position: Readonly<XYPosition>, snapToGridAmount: number): boolean =>
    position.x % snapToGridAmount === 0 && position.y % snapToGridAmount === 0
...
export const setSelected = <T extends { selected?: boolean }>(selectable: readonly T[], selected: boolean): T[] =>
    selectable.map((s) => ({ ...s, selected }))
